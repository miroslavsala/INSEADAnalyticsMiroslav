...
if("xgboost" %in% rownames(installed.packages()) == FALSE) {install.packages("xgboost")} # Check if you have universal installer package, install if not
if("randomForest" %in% rownames(installed.packages()) == FALSE) {install.packages("randomForest")} # Check if you have universal installer package, install if not
...
ctree_tree<-ctree(default.payment.next.month~ LIMIT_BAL + SEX + EDUCATION + MARRIAGE + AGE + 
                    ...
                    MONTHS_ABOVE_LIMIT + MONTHS_WITH_NEG1 + MONTHS_WITH_NEG2 +
                    MONTHS_WITH_0 + MONTHS_WITH_1 + MONTHS_WITH_2 + MONTHS_WITH_3 +
                    MONTHS_WITH_4 + MONTHS_WITH_5 + MONTHS_WITH_6 + MONTHS_WITH_7 + MONTHS_WITH_8 +
                    DELTA_LAST_MONTH + DELTA_LAST_2MONTHS + DELTA_LAST_3MONTHS,
                  data=training ) #Run ctree on training data
